...
**重点：** 在JS中，函数也是对象，并且函数本身都会带有一个原型对象`prototype`，通过这个函数创建的对象，也会有一个隐式原型`__proro__`，会指向函数的原型对象，因此可以得出一下关系图。（红线代表 new 的关系）

![proto01](/javascript/proto/proto01.jpg)
...
![proto02](/javascript/proto/proto02.jpg)
...
![proto03](/javascript/proto/proto03.jpg)

`Function`的原型是一个对象，那么又可以得出一下关系：

![proto04](/javascript/proto/proto04.jpg)

此时再回顾最开始的`Foo`函数，`Foo`也是通过`new Function`创建出来的，由此得出原型链的最终关系图：

![proto05](/javascript/proto/proto05.jpg)
